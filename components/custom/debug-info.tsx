...
export function DebugInfo() {
  ...
  const debugInfo: DebugInfo = {
    ...
    lastDeployTime: process.env.NEXT_PUBLIC_VERCEL_DEPLOYED_AT || new Date().toISOString(),
    nodeVersion: process.version,
    systemInfo: {
      screen: `${width}x${height}`,
      userAgent: typeof window !== 'undefined' ? window.navigator.userAgent : '',
      memory: typeof window !== 'undefined' ? `${(performance?.memory?.usedJSHeapSize || 0) / 1024 / 1024}MB` : '',
      cpu: typeof window !== 'undefined' ? navigator?.hardwareConcurrency?.toString() || '' : ''
    }
  }
          ...
          >
            <div className="space-y-2 text-xs">
              ...
              <div className="flex justify-between">
                ...
                <span>{new Date(debugInfo.lastDeployTime).toLocaleString()}</span>
              </div>
              <div className="flex justify-between">
