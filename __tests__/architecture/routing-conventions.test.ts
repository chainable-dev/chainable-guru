...
describe('Next.js Routing Conventions', () => {
  ...
  it('follows route group naming conventions', () => {
    ...
    routeGroups.forEach(group => {
      ...
      expect(
        fs.existsSync(path.join(appDir, group.name, 'layout.tsx'))
      ).toBe(true)
    })
  })
  ...
  it('has proper dynamic route segments', () => {
    ...
    if (fs.existsSync(chatDir)) {
      const dynamicRoutes = fs.readdirSync(chatDir, { withFileTypes: true })
      ...
      dynamicRoutes.forEach(route => {
        ...
        expect(
          fs.existsSync(path.join(chatDir, route.name, 'page.tsx'))
        ).toBe(true)
      })
    }
  })
  ...
  it('has proper API route structure', () => {
    ...
    if (fs.existsSync(apiDir)) {
      ...
      apiRoutes.forEach(route => {
        ...
        expect(
          fs.existsSync(path.join(apiDir, route.name, 'route.ts'))
        ).toBe(true)
      })
    }
  })
